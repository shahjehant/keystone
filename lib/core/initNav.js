...
module.exports = function initNav (sections, req) {
	var keystone = this;
	...
	_.forEach(sections, function (section, key) {
		...
		section.lists = _.map(section.lists, function (i) {
			if (typeof i === 'string') {
				...
				if (_.isFunction(list.options.hidden) ? list.options.hidden(req) : list.options.hidden) {
		...
		section.lists = _.compact(section.lists);
