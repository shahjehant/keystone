...
module.exports = function IndexRoute(req, res) {
	...
	var keystoneData = {
		...
		user: {
			...
			companyName: req.user.client_id ? req.user.client_id.name : "",
		},
		...
		wysiwyg: {
			options: {
				...
				enableCloudinaryUploads: keystone.get(
				...
				enableS3Uploads: keystone.get("wysiwyg s3 images")
					...
					: false,
				additionalButtons:
					keystone.get("wysiwyg additional buttons") || "",
				additionalPlugins:
					keystone.get("wysiwyg additional plugins") || "",
				additionalOptions:
					keystone.get("wysiwyg additional options") || {},
	...
	keystoneData.csrf.header[keystone.security.csrf.CSRF_HEADER_KEY] =
